...
void Cube::loadObj(std::string_view path, bool standardize) {
  tinyobj::ObjReader reader;

  if (!reader.ParseFromFile(path.data())) {
    if (!reader.Error().empty()) {
      throw abcg::RuntimeError(
          fmt::format("Failed to load Cube {} ({})", path, reader.Error()));
    }
    throw abcg::RuntimeError(fmt::format("Failed to load Cube {}", path));
  }
...
void Cube::render(int numTriangles) const {
...
void Cube::setupVAO(GLuint program) {
