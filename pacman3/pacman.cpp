...
void Pacman::loadObj(std::string_view path, bool standardize) {
  tinyobj::ObjReader reader;

  if (!reader.ParseFromFile(path.data())) {
    if (!reader.Error().empty()) {
      throw abcg::RuntimeError(
          fmt::format("Failed to load Pacman {} ({})", path, reader.Error()));
    }
    throw abcg::RuntimeError(fmt::format("Failed to load Pacman {}", path));
  }
...
void Pacman::render(int numTriangles) const {
...
void Pacman::setupVAO(GLuint program) {
